...
public class CardGridPlacer : MonoBehaviour
{
    ...
    private void Update()
    {
        // FIXED: Pure rotation only. No more refund logic interrupting it!
    ...
    public void ProcessDragUpdate()
    {
        ...
        if (Physics.Raycast(ray, out RaycastHit hit, Mathf.Infinity, gridLayerMask))
        {
            ...
            if (tile != null)
            {
                ...
                active3DPreview.transform.position = Vector3.Lerp(active3DPreview.transform.position, hit.point, Time.deltaTime * 20f);
                UpdateGridHighlighting();
            }
        }
        else
    ...
    public bool AttemptDrop()
    {
        ...
        // 1. Drag & Drop physical Inventory Refund 
        ...
        if (active3DPreview != null)
        {
            if (Physics.Raycast(ray, out RaycastHit hit, Mathf.Infinity, gridLayerMask))
            {
                ...
                if (tile != null)
                {
                    ...
                    {
                        bool willCook = false;
                        if (ingredientData.isCookable && tile.parentStation.stationType == StationType.Stove)
                        {
                            willCook = true;
                        }
